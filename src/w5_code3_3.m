% Q3_3
% Display zero-mean random numbers' mean and variance
% ==================

function w5_code3_3(minNum, maxNum, n, fps)
    function mean_hist = genMeanHist(minNum, maxNum)
        ...
        mean_hist(i) = mean(w5_code3_0(i, -0.5, "F"));
        end
    end
    function var_hist = genVarHist(minNum, maxNum)
        ...
        for i = 1:1:arraySize(2)
        var_hist(i) = var(code3_0(i, -0.5, "F"));
        end
    end
    ...
    % Initialize video
    filename = strcat("Zero-mean_", int2str(minNum), "_", int2str(maxNum), "_", int2str(n), ".avi");
